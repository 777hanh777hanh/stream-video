...
const StreamListVideo: FC<StreamListVideoProps> = ({ videoTagsSlug, videoSlug }) => {
    ...
    const renderListVideo = () => {
        if (sortListVideo && sortListVideo.length > 0) {
            return sortListVideo.map((video: { [key: string]: any }, index) => {
                const isCurrentVideo = videoSlug === video.slug;

                return (
                    <div
                        key={video.id || index}
                        className={cx('stream__list-item', { 'stream__list-item--active': isCurrentVideo })}
                    >
                        {/* Index */}
    ...
    return <div className={cx('stream__list')}>{renderListVideo()}</div>;
};
